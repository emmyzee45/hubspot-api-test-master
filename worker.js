...
const processMeetings = async (domain, hubId, q) => {
  ...
  // Update the Domain schema to include meetings in lastPulledDates if not already present
  if (!account.lastPulledDates.meetings) {
    account.lastPulledDates.meetings = moment().subtract(4, 'year').toISOString();
  }
  ...
  while (hasMore) {
    ...
    const attendeesResults = (await (await hubspotClient.apiRequest({
      method: 'post',
      path: '/crm/v3/associations/MEETINGS/CONTACTS/batch/read',
      body: { inputs: meetingIds.map(meetingId => ({ id: meetingId })) }
    })).json())?.results || [];

    // Create a mapping of meeting IDs to contact IDs
    const meetingAttendees = {};
    attendeesResults.forEach(result => {
      if (result.from && result.to && result.to.length > 0) {
        meetingAttendees[result.from.id] = result.to.map(contact => contact.id);
      }
    });

    // Fetch contact details for attendees
    const contactDetails = {};
    for (const contactId of Object.values(meetingAttendees).flat()) {
      try {
        const contactResult = await hubspotClient.crm.contacts.basicApi.getById(contactId, ['email']);
        contactDetails[contactId] = contactResult.properties.email;
      } catch (err) {
        console.error(`Failed to fetch contact details for ID ${contactId}:`, err);
      }
    }
    ...
    data.forEach(meeting => {
      ...
      // Get attendee emails for this meeting
...
// Update the pullDataFromHubspot function to include meetings processing
